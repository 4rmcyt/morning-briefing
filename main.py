...
def get_calgary_weather():
    ...
    try:
        ...
        return (
            f"Weather report. Currently it is {current_temp} degrees."
            f" Today's high will be {max_temp} and the low will be {min_temp}."
        )
...
def get_today_events():
    ...
    try:
        client = caldav.DAVClient(
            url=RADICALE_URL, username=RADICALE_USER, password=RADICALE_PASS
        )
...
if __name__ == "__main__":
    full_briefing = (
        f"Good morning! {get_calgary_weather()} {get_today_events()}"
    )
